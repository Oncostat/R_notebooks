...
L'intervalle de confiance permet de mesurer l'incertitude de l'estimateur de Kaplan-Meier à chaque temps d'évènement, et est une représentation de l'incertitude adaptée si l'intérêt de l'étude se porte sur un temps particulier (exemple : survie à 5 ans).
...
Elle permet de mesurer l'incertitude sur toute la courbe de survie. C'est une représentation de l'incertitude adaptée si l'intérêt de l'étude se porte sur l'ensemble du suivi. Elle peut donc être associée comme représentation graphique en parallèle du test du log-rank (qui compare la survie entre 2 groupes en prenant en compte tout le suivi).
Pour cela, nous utiliserons la méthode de Hall-Wellner avec transformation logarithmique :

```{r}
HWCB <- km.ci(KM, conf.level=0.95, method="loghall") 
summary(HWCB)
```
...
La bande de confiance n'est malheureusement pas supportée par défaut par ce package. Un code plus complexe sera proposé dans une version ultérieure de ce notebook.
